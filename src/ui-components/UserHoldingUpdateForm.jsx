...
import { getUserHolding } from "../graphql/queries";
import { updateUserHolding } from "../graphql/mutations";
...
export default function UserHoldingUpdateForm(props) {
  const {
    id: idProp,
    userHolding: userHoldingModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userHoldingRecord
      ? { ...initialValues, ...userHoldingRecord }
      : initialValues;
    setStocks(cleanValues.stocks ?? []);
  ...
  const [userHoldingRecord, setUserHoldingRecord] =
    React.useState(userHoldingModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserHolding.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserHolding
        : userHoldingModelProp;
      setUserHoldingRecord(record);
    };
    queryData();
  }, [idProp, userHoldingModelProp]);
  React.useEffect(resetStateValues, [userHoldingRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateUserHolding.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userHoldingRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserHoldingUpdateForm")}
      {...rest}
    >
      <ArrayField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userHoldingModelProp)}
